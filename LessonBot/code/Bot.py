...
course_Type = ['博雅通識', '博雅通識']  # 可選:基礎通識,博雅通識 格式:[第一門課的資訊,第二門課的資訊]
# 可選:中國語文知識與應用,英文能力訓練,資訊能力課程,基礎概論課程, 格式:[第一門課的資訊,第二門課的資訊]
course_Dimention = ['能源、環境與生態', '能源、環境與生態']
#     跨向度課程,藝術與美學,能源、環境與生態,人文思維與生命探索,公民與社會參與,經濟與國際脈動,自然科學與技術
course_Page = [1, 2]  # 要搶的課程在第幾頁 格式:[第一門課的資訊,第二門課的資訊]
course_Position = [10, 3]  # 要搶的課程在該頁的位置 格式:[第一門課的資訊,第二門課的資訊]
...
def Login_User_Page():
    ...
    try:
        ...
        time.sleep(take_a_break)
        driver.find_element(
            'xpath', '/html/body/font/center/form/table/tbody/tr[6]/td/input[1]').submit()  # 送出密碼
        time.sleep(take_a_break)
    except:
...
def Try_Get_Course(i):
    ...
    try:
        ...
        time.sleep(take_a_break)
        driver.find_element('xpath',
                            '/html/body/center/form/table/tbody/tr[2]/th/input').submit()  # 送出表單
        time.sleep(take_a_break)
        driver.close()
        time.sleep(take_a_break)
    except:
